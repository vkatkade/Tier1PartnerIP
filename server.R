...
shinyServer(function(input, output) {    
  ...
  output$distPlot <- renderPlot({
    df.m <- partnerAnalysis(input$select)
#    ggplot(df.m, aes(x=technology, y=value, fill=credit.type)) + geom_bar(stat='identity', color='white')      
    ggplot(df.m, aes(x=credit.type, y=value, fill=credit.type)) + geom_bar(stat='identity', color='white')      
})

  output$table <- renderDataTable({
    df.m <- partnerAnalysis(input$select)
    df.m <- df.m[,c("credit.type", "value")]
    df.m
  })
